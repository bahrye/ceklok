...
function verifyLogin(username, password) {
  try {
    ...
    const data = usersSheet.getRange(2, 1, usersSheet.getLastRow() -1, 4).getValues(); // Username, Password, Role, Nama_Guru_Asli
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (rowUsername === username && rowPassword === password) {
        ...
        try {
            const ss = SpreadsheetApp.getActiveSpreadsheet(); // Definisikan ss jika belum ada di scope ini
            ...
            if (!loginHistorySheet) {
                ...
                // *** MODIFIKASI: Tambah header Nama_Guru ***
                loginHistorySheet.appendRow(["Username", "Login_Timestamp", "Nama_Guru"]);
                loginHistorySheet.getRange("A1:C1").setFontWeight("bold"); // Opsional: buat header tebal
            }
            const timestamp = new Date();
            // *** MODIFIKASI: Tambahkan effectiveTeacherName ke appendRow ***
            // Gunakan '' jika effectiveTeacherName null/undefined agar sel tidak kosong total
            loginHistorySheet.appendRow([rowUsername, timestamp, effectiveTeacherName || '']);
        } catch (logError) {
            Logger.log(`Gagal mencatat histori login untuk ${rowUsername}: ${logError.toString()}`);
            // Jangan gagalkan proses login hanya karena gagal mencatat histori
        }
        return { success: true, username: rowUsername, role: rowRole, teacherName: effectiveTeacherName };
      }
    }
...
function ensureTemplateSheetExists() {
  ...
  if (!templateSheet) {
    ...
    const worksheetHeaders = [
      "EXPORT_ID", "Tanggal (YYYY-MM-DD)", "Hari (Bahasa Indonesia)",
      "Jam Masuk (HH:MM)", "Jam Pulang (HH:MM)",
      "Keterangan Libur 1", "Keterangan Libur 2" // Keterangan Libur 1 dan 2 bisa sama (keteranganLiburManual)
    ];
...
function saveOrUpdateLockId(teacherName, lockId) {
  ...
  try {
    const lockIdSheet = ensureSheet(LOCK_ID_SHEET_NAME, ["Nama_Guru", "LOCK_ID"]); // LOCK_ID_SHEET_NAME adalah variabel global
    const data = lockIdSheet.getDataRange().getValues();
    let rowIndex = -1;
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === teacherName) {
