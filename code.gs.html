...
const CHANGE_EMAIL_OTP_EXPIRY_MINUTES_GS = 10; // Jika Anda juga menggunakan ini di GS untuk fitur ganti email
const PASSWORD_RESET_OTP_EXPIRY_MINUTES_GS = 10; // Jika Anda juga menggunakan ini di GS untuk fitur reset password

const REMINDER_LOG_SHEET_NAME = "Reminder_Log"; // Untuk melacak notifikasi yang sudah dikirim
const EMAIL_SENDER_NAME = "Admin Ceklok MTS Tanuntung"; // Nama pengirim email
...
function verifyLogin(usernameFromInput, password) { // Mengganti nama parameter 'username' menjadi 'usernameFromInput' agar lebih jelas
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    if (usernameColIdx === -1 || passwordColIdx === -1 || roleColIdx === -1 || primaryEmailColIdx === -1) { // Pastikan primaryEmailColIdx juga ada
        let missingCols = [];
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (inputIsEmailFormat) {
        // Jika input adalah format email, cek PrimaryEmail dulu, lalu Username
        if (sheetPrimaryEmail.toLowerCase() === usernameFromInput.toLowerCase()) {
          ...
          // Bisa jadi username lamanya adalah email, atau PrimaryEmail belum diisi tapi username sudah email
          match = true;
        }
      } else {
        // Jika input bukan format email, hanya cek kolom Username
      ...
      if (match && sheetPassword === password) {
        userRowData = currentRow; // Simpan seluruh baris data pengguna yang cocok
        break; 
      }
    }

    if (userRowData) {
      const actualUsername = String(userRowData[usernameColIdx]).trim(); // Username asli dari kolom Username
      ...
      if (role === "user") {
        ...
        // Validasi Nama_Guru_Asli terhadap sheet Nama_Guru (kode ini seharusnya sudah ada)
        ...
        if (sheetGuru) {
            ...
            // Mungkin tidak kritis jika sheet nama guru hanya untuk dropdown admin,
            // tapi konsistensi data penting.
      ...
      try {
          ...
          // Catat username asli, bukan input login (jika berbeda)
      ...
      return {
          success: true,
          username: actualUsername, // Selalu kembalikan username asli dari kolom "Username"
          role: role,
    ...
    return { success: false, error: "Username/Email atau password salah." }; // Pesan error lebih generik
  } catch (e) {
...
// GANTI FUNGSI getCeklokData YANG LAMA DENGAN YANG INI
...
function getCeklokData(nama, bulanTahun) {
  try {
    ...
    if (settings.error) {
      Logger.log(`getCeklokData: Peringatan - Tidak dapat memuat settings untuk validasi bulan - ${settings.error}`);
    } else {
      // Parse helper
      ...
      if (requestedDateObj && minDateObj && requestedDateObj < minDateObj) {
        Logger.log(`getCeklokData: Periode ${bulanTahun} tidak diizinkan. Batas minimum: ${settings.adminDefinedMinMonthYear}.`);
      ...
      if (requestedDateObj && maxDateObj && requestedDateObj > maxDateObj) {
        Logger.log(`getCeklokData: Periode ${bulanTahun} tidak diizinkan. Batas maksimum: ${settings.serverCurrentMonthYear}.`);
    ...
    if (!sheet) {
      Logger.log("getCeklokData: Sheet 'Ceklok_Manual' tidak ditemukan.");
      return { error: `Sheet '${CEKLOK_MANUAL_SHEET_NAME}' tidak ditemukan.` }; //
    }
    if (!liburSheet) {
      Logger.log("getCeklokData: Sheet 'Kalender_Pendidikan' tidak ditemukan.");
      return { error: `Sheet '${KALENDER_SHEET_NAME}' tidak ditemukan.` }; //
    }
    ...
    if (opsiKetLiburSheet && opsiKetLiburSheet.getLastRow() > 1) {
      ...
      Logger.log(`getCeklokData: Sheet '${OPSIKETLIBUR_SHEET_NAME}' tidak ada atau kosong.`); //
    }
    ...
    if (!bulanTahun || !/^\d{2}\/\d{4}$/.test(bulanTahun)) {
      Logger.log(`getCeklokData: Format bulanTahun tidak valid: ${bulanTahun}`);
      return { error: "Format bulanTahun tidak valid (mm/yyyy)." };
    }
    const [mmStr, yyyyStr] = bulanTahun.split("/");
    const mm = parseInt(mmStr, 10);
    const yyyy = parseInt(yyyyStr, 10);
    if (isNaN(mm) || isNaN(yyyy) || mm < 1 || mm > 12) {
      Logger.log(`getCeklokData: Nilai Bulan/Tahun tidak valid: ${bulanTahun}`);
    ...
    if ([namaColIdx, tanggalColIdx, masukColIdx, pulangColIdx, ketLiburManualColIdx].includes(-1)) {
      ...
      Logger.log(`getCeklokData: Header di 'Ceklok_Manual' tidak lengkap. Hilang: ${missingHeaders.join(', ')}`);
    ...
    for (let i = 1; i <= lastDay; i++) {
      ...
      const isLiburKalender = (dateObj.getDay() === 0) || (keteranganKalender !== ''); // Minggu atau libur kalender

      const dataRow = allData.find(r =>
        String(r[namaColIdx]).trim() === nama &&
        r[tanggalColIdx] instanceof Date && // Pastikan ini objek Date
      ...
      resultData.push({
        ...
        keterangan: keteranganKalender, // Keterangan dari Kalender_Pendidikan
        keteranganLiburManual: dataRow && dataRow[ketLiburManualColIdx] !== undefined ? String(dataRow[ketLiburManualColIdx]) : '' // Keterangan dari Ceklok_Manual
      });
    }

    // Ambil daftar bulan yang terkunci
    const lockStatusResult = getLockedMonths(); // Memanggil fungsi yang sudah dibuat
    ...
    if (lockStatusResult.success) {
      ...
      Logger.log(`getCeklokData: Untuk bulan ${bulanTahun}, status terkunci: ${isMonthCurrentlyLocked}. Daftar terkunci: ${(lockStatusResult.months || []).join(', ')}`);
    } else {
      Logger.log(`getCeklokData: Peringatan - Gagal memeriksa status kunci bulan ${bulanTahun} - ${lockStatusResult.error}`);
      // Jika gagal memeriksa status kunci, Anda bisa memutuskan apakah defaultnya terkunci atau tidak.
      // Untuk keamanan, bisa diasumsikan terkunci jika gagal cek, atau biarkan false dan log error.
      // isMonthCurrentlyLocked = true; // Contoh jika ingin default terkunci saat gagal cek
    }

    return { 
      data: resultData, 
      optionsKeteranganLibur: ketLiburOptions, 
      isLocked: isMonthCurrentlyLocked // Menyertakan status kunci bulan
    };
  } catch (e) {
    Logger.log(`Error in getCeklokData (Nama: ${nama}, BulanTahun: ${bulanTahun}): ${e.toString()}\nStack: ${e.stack}`);
...
function simpanData(nama, dataInput) {
  try {
    // --- AWAL PENAMBAHAN PROTEKSI ---
    if (!dataInput || dataInput.length === 0 || !dataInput[0].tanggal) {
      Logger.log("simpanData dipanggil dengan dataInput tidak valid.");
      return "Error: Data input tidak valid untuk disimpan.";
    }
    // Ambil bulanTahun dari data pertama (asumsi semua data dalam satu bulan)
    const tanggalPertama = dataInput[0].tanggal; // Format "dd/MM/yyyy"
    const parts = tanggalPertama.split("/");
    if (parts.length !== 3) {
      Logger.log(`simpanData: Format tanggalPertama tidak valid: ${tanggalPertama}`);
      return "Error: Format tanggal pada data input tidak valid.";
    }
    const bulanTahunSimpan = `${parts[1]}/${parts[2]}`; // Format "MM/YYYY"
    Logger.log(`simpanData: Mencoba menyimpan untuk bulan ${bulanTahunSimpan}. Nama Guru: ${nama}`);

    const lockStatusResult = getLockedMonths();
    Logger.log(`simpanData: Hasil getLockedMonths: ${JSON.stringify(lockStatusResult)}`);

    if (lockStatusResult.success && (lockStatusResult.months || []).includes(bulanTahunSimpan)) {
      Logger.log(`simpanData: PENYIMPANAN DITOLAK. Bulan ${bulanTahunSimpan} terkunci. Daftar terkunci: ${(lockStatusResult.months || []).join(', ')}`);
      return `Error: Bulan ${bulanTahunSimpan} telah dikunci oleh Admin dan tidak dapat diubah.`;
    } else if (!lockStatusResult.success) {
      Logger.log(`simpanData: Peringatan - Gagal memeriksa status kunci bulan - ${lockStatusResult.error}. Penyimpanan dilanjutkan (KEBIJAKAN SAAT INI).`);
      // Anda bisa mengubah ini untuk return error jika pengecekan gagal:
      // return `Error: Gagal memverifikasi status penguncian bulan. Silakan coba lagi.`;
    } else {
      Logger.log(`simpanData: Bulan ${bulanTahunSimpan} tidak terkunci. Daftar terkunci: ${(lockStatusResult.months || []).join(', ')}. Penyimpanan dilanjutkan.`);
    }
    // --- AKHIR PENAMBAHAN PROTEKSI ---

    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName(CEKLOK_MANUAL_SHEET_NAME); //
    if (!sheet) {
      Logger.log("simpanData: Sheet 'Ceklok_Manual' tidak ditemukan.");
      return "Error: Sheet 'Ceklok_Manual' tidak ditemukan.";
    }
    ...
    if ([namaColIdx, tanggalColIdx, masukColIdx, pulangColIdx, ketLiburManualColIdx].includes(-1)) {
      ...
      if (pulangColIdx === -1) missingHeaders.push("Jam_Pulang");
      if (ketLiburManualColIdx === -1) missingHeaders.push("Ket_Libur_Manual");
      Logger.log(`simpanData: Header di 'Ceklok_Manual' tidak lengkap. Hilang: ${missingHeaders.join(', ')}`);
      return `Error: Header di 'Ceklok_Manual' tidak lengkap. Kolom yang hilang: ${missingHeaders.join(', ')}.`;
    }

    dataInput.forEach(row => {
      if (!row || !row.tanggal) return; // Lewati jika baris tidak valid
      ...
      if (existingRowIndex !== -1) {
      ...
      } else if (jm || jp || ketLiburTrimmed !== "") { // Hanya append jika ada data yang diisi
        const dateParts = tanggal.split("/");
        // Pastikan dateParts valid sebelum membuat objek Date
        if (dateParts.length === 3) {
            ...
            Logger.log(`simpanData: Format tanggal tidak valid saat appendRow untuk tanggal ${tanggal}`);
        }
      }
    });
    ...
    if (sheet.getLastRow() > 1) {
      ...
      // Pastikan kolom ada sebelum setNumberFormat
      if (masukColIdx > -1 && lastDataRow > 1) sheet.getRange(2, masukColIdx + 1, lastDataRow - 1, 1).setNumberFormat("@");
      if (pulangColIdx > -1 && lastDataRow > 1) sheet.getRange(2, pulangColIdx + 1, lastDataRow - 1, 1).setNumberFormat("@");
      if (ketLiburManualColIdx > -1 && lastDataRow > 1) sheet.getRange(2, ketLiburManualColIdx + 1, lastDataRow - 1, 1).setNumberFormat("@");
    }
    Logger.log(`simpanData: Data berhasil disimpan untuk guru ${nama}, bulan ${bulanTahunSimpan}.`);
    return "Data berhasil disimpan.";
  } catch (e) {
    Logger.log(`Error in simpanData (Nama: ${nama}, DataInput: ${JSON.stringify(dataInput)}): ${e.toString()}\nStack: ${e.stack}`);
...
function checkAndSendAttendanceReminders() {
  ...
  // Pastikan sheet log ada, jika tidak, buat dengan header yang benar
  const reminderLogSheet = ensureSheet(REMINDER_LOG_SHEET_NAME, [
    "Nama_Guru", 
    "Target_Tanggal_Absen", // Tanggal yang kosong atau tidak lengkap
    "Tipe_Notifikasi",      // "berkala_kosong" atau "tidak_lengkap"
    "Tanggal_Notifikasi_Terkirim", 
    "Urutan_Pengingat_Berkala" // Untuk tipe "berkala_kosong" (1, 2, 3, ...)
  ]);

  if (!usersSheet || !ceklokManualSheet || !kalenderSheet || !settingSheet) {
    Logger.log("Peringatan: Salah satu sheet penting (Users, Ceklok_Manual, Kalender_Pendidikan, Setting) tidak ditemukan. Proses pengingat dibatalkan.");
    return;
  }

  // 1. Ambil data guru aktif (Nama_Guru_Asli, PrimaryEmail)
  const usersData = usersSheet.getDataRange().getValues();
  const userHeaders = usersData[0].map(h => String(h).trim().toLowerCase());
  const idxUserUsername = userHeaders.indexOf("username"); // Untuk referensi log
  ...
  if ([idxUserTeacherName, idxUserPrimaryEmail, idxUserRole, idxUserUsername].includes(-1)) {
    Logger.log("Error: Kolom penting (username, nama_guru_asli, primaryemail, role) di sheet Users tidak ditemukan. Proses pengingat dibatalkan.");
    return;
  }
  ...
  for (let i = 1; i < usersData.length; i++) {
    ...
    // const username = usersData[i][idxUserUsername] ? String(usersData[i][idxUserUsername]).trim() : ""; // Jika diperlukan
  ...
  if (activeTeachers.length === 0) {
    Logger.log("Tidak ada guru aktif dengan email valid yang ditemukan untuk pengingat.");
    return;
  }

  // 2. Ambil data kalender libur
  const liburMap = helper_getLiburMap(kalenderSheet);

  // 3. Ambil semua data dari Ceklok_Manual untuk efisiensi
  ...
  if ([idxCeklokNama, idxCeklokTanggal, idxCeklokMasuk, idxCeklokPulang, idxCeklokKetManual].includes(-1)) {
    Logger.log("Error: Kolom penting di sheet Ceklok_Manual tidak lengkap. Proses pengingat dibatalkan.");
    return;
  }
  
  // 4. Ambil batas bawah periode dari Setting A1
  const adminMinMonthYear = settingSheet.getRange('A1').getDisplayValue();
  let globalMinDateToCheck; // Tanggal paling awal kita peduli untuk cek kekosongan
  if (adminMinMonthYear && /^\d{2}\/\d{4}$/.test(adminMinMonthYear)) {
    const [mm, yyyy] = adminMinMonthYear.split('/');
    globalMinDateToCheck = new Date(parseInt(yyyy), parseInt(mm) - 1, 1);
  } else {
    // Default 2 bulan lalu dari hari ini jika setting tidak ada/valid
    globalMinDateToCheck = new Date(today.getFullYear(), today.getMonth() - 2, 1);
    Logger.log(`Peringatan: Setting A1 (mm/yyyy) tidak valid atau tidak ada, menggunakan default 2 bulan lalu: ${Utilities.formatDate(globalMinDateToCheck, Session.getScriptTimeZone(), "MM/yyyy")}.`);
  }
  globalMinDateToCheck.setHours(0,0,0,0);


  // --- Mulai iterasi per guru ---
  activeTeachers.forEach(teacher => {
    Logger.log(`Memeriksa pengingat absensi untuk guru: ${teacher.name} (${teacher.email})`);

    // A. Logika untuk Pengingat Absensi Tidak Lengkap (Harian)
    processIncompleteAttendanceReminder(teacher, allCeklokData, ceklokHeaders, liburMap, reminderLogSheet, today, globalMinDateToCheck);

    // B. Logika untuk Pengingat Pengisian Absensi Berkala (Kosong)
    processPeriodicEmptyAttendanceReminder(teacher, allCeklokData, ceklokHeaders, liburMap, reminderLogSheet, today, globalMinDateToCheck);
  });

  Logger.log("Proses pemeriksaan pengingat absensi selesai.");
}
  ...
  for (let i = 1; i < allCeklokData.length; i++) {
    ...
    if (namaGuruDiSheet === teacher.name) {
      ...
      if (tanggalAbsen >= globalMinDateToCheck && tanggalAbsen <= today) {
        ...
        // Hanya proses jika BUKAN hari libur berdasarkan kalender DAN TIDAK ADA keterangan manual yang sudah diisi
        // (karena jika ada ket. manual, berarti sudah ada tindakan dari guru).
...
function processPeriodicEmptyAttendanceReminder(teacher, allCeklokData, ceklokHeaders, liburMap, reminderLogSheet, today, globalMinDateToCheck) {
  ...
  // Cari tanggal terakhir guru tersebut mengisi absensi (ada jam masuk/pulang ATAU keterangan manual)
  // Mulai dari data terbaru ke belakang, tapi tidak lebih awal dari globalMinDateToCheck
  for (let i = allCeklokData.length - 1; i >= 1; i--) {
    ...
    if (String(row[idxCeklokNama]).trim() === teacher.name) {
      ...
      if (tanggalCek < globalMinDateToCheck) continue; // Jangan periksa tanggal sebelum batas minimum

      const jamMasuk = row[idxCeklokMasuk] ? String(row[idxCeklokMasuk]).trim() : "";
      const jamPulang = row[idxCeklokPulang] ? String(row[idxCeklokPulang]).trim() : "";
      const ketLiburManualValue = row[idxCeklokKetManual] ? String(row[idxCeklokKetManual]).trim() : "";

      if ((jamMasuk || jamPulang) || ketLiburManualValue) { // Dianggap terisi jika ada jam ATAU keterangan
  ...
  if (lastEffectivelyFilledDate) {
    ...
    dateToCheckFrom.setDate(lastEffectivelyFilledDate.getDate() + 1); // Mulai cek dari hari setelah terakhir diisi
  } else {
    // Jika guru belum pernah mengisi sama sekali (setelah globalMinDateToCheck)
    dateToCheckFrom = new Date(globalMinDateToCheck);
    Logger.log(`Guru ${teacher.name} belum ada catatan absensi valid sejak ${Utilities.formatDate(globalMinDateToCheck, Session.getScriptTimeZone(), "dd/MM/yyyy")}. Pemeriksaan kekosongan dimulai dari tanggal tersebut.`);
  }
  dateToCheckFrom.setHours(0,0,0,0);


  // Iterasi dari dateToCheckFrom hingga kemarin (karena hari ini belum tentu wajib diisi saat script jalan)
  ...
  while(tempDate <= yesterday) {
    ...
    // Untuk helper_isWorkingDay, kita perlu ket manual spesifik untuk tanggal `tempDate` jika ada
    ...
    if (helper_isWorkingDay(tempDate, liburMap, ketManualForTempDate) && !isAlreadyFilledOrHasKet) {
      ...
      break; // Temukan hari kerja pertama yang kosong
    }
    tempDate.setDate(tempDate.getDate() + 1);
  }

  if (!firstEmptyWorkingDay) {
    Logger.log(`Tidak ditemukan hari kerja yang kosong untuk guru ${teacher.name} sampai dengan kemarin.`);
    return; // Tidak ada yang perlu diingatkan
  }

  // `firstEmptyWorkingDay` adalah tanggal pertama yang kosong dan seharusnya diisi.
  // `lastEffectivelyFilledDate` adalah tanggal terakhir guru melakukan pengisian.
  // Jika tidak ada `lastEffectivelyFilledDate` (guru baru / belum pernah isi),
  // maka `firstEmptyWorkingDay` adalah tanggal paling awal yang kosong sejak `globalMinDateToCheck`.
  // Dalam kasus guru baru, `baseDateForReminder` adalah sehari sebelum `firstEmptyWorkingDay`.
  
  const baseDateForReminder = lastEffectivelyFilledDate ? new Date(lastEffectivelyFilledDate) : new Date(firstEmptyWorkingDay.setDate(firstEmptyWorkingDay.getDate()-1));
  baseDateForReminder.setHours(0,0,0,0);
  
  const daysSinceBase = Math.floor((today.getTime() - baseDateForReminder.getTime()) / (1000 * 60 * 60 * 24));
  const targetEmptyDateStr = Utilities.formatDate(firstEmptyWorkingDay, Session.getScriptTimeZone(), "dd/MM/yyyy");

  let reminderType = "berkala_kosong"; // Generic type, detailnya di urutan
  let urutanPengingat = 0;
  const lastReminderForTarget = helper_getLastPeriodicNotificationForTarget(reminderLogSheet, teacher.name, targetEmptyDateStr, reminderType);

  if (!lastReminderForTarget) { // Belum pernah ada notifikasi untuk target tanggal kosong INI
    if (daysSinceBase >= 3) {
      urutanPengingat = 1; // Notifikasi pertama (setelah 3 hari dari basis)
    }
  } else {
    const lastReminderDate = new Date(lastReminderForTarget.tanggalKirim);
    ...
    const currentSequence = lastReminderForTarget.urutan;

    if (currentSequence === 1 && daysSinceBase >= 5 && daysSinceLastReminderForTarget >= (5-3) ) { // Dari urutan 1 ke 2 (selisih 2 hari dari notif terakhir, total 5 dari basis)
      urutanPengingat = 2;
    } else if (currentSequence >= 2 && daysSinceBase >= (5 + (currentSequence - 1) * 5) && daysSinceLastReminderForTarget >= 5) { // Urutan ke-2 dst. (selisih 5 hari dari notif terakhir)
      urutanPengingat = currentSequence + 1;
    }
  }

  if (urutanPengingat > 0) {
    if (!helper_hasSentNotificationToday(reminderLogSheet, teacher.name, targetEmptyDateStr, `${reminderType}_${urutanPengingat}`)) {
        ...
        Logger.log(`Mengirim notifikasi pengisian berkala (Urutan: ${urutanPengingat}) ke ${teacher.name}. Absen terakhir diisi: ${actualLastFilledDateStr}. Target kosong: ${targetEmptyDateStr}. Selisih dari basis: ${daysSinceBase} hari.`);
        const subject = `Pemberitahuan: Mohon Lengkapi Absensi Ceklok - ${teacher.name}`;
        let body = `Yth. ${teacher.name},\n\n`;
        ...
        body += `Mohon untuk segera mengisi data absensi Anda melalui aplikasi Ceklok di link : https://tiny.cc/ceklok-mts.\n\nTerima kasih.\n${EMAIL_SENDER_NAME}`;
        try {
            MailApp.sendEmail({
            to: teacher.email,
            subject: subject,
            ...
            helper_logSentNotification(reminderLogSheet, teacher.name, targetEmptyDateStr, `${reminderType}_${urutanPengingat}`, urutanPengingat);
        } catch (e) {
            Logger.log(`Gagal mengirim email pengingat berkala ke ${teacher.email} untuk ${targetEmptyDateStr}: ${e.toString()}`);
        }
    } else {
        Logger.log(`Notifikasi pengisian berkala (Urutan: ${urutanPengingat}) untuk ${teacher.name} target ${targetEmptyDateStr} sudah dikirim hari ini.`);
    }
  }
}
 ...
 * @param {string} targetDateStr Tanggal target absensi (dd/MM/yyyy).
 * @param {string} notifType Tipe notifikasi (misal "tidak_lengkap", "berkala_kosong_1").
 * @param {number} sequence Urutan pengingat untuk tipe berkala.
 */
function helper_logSentNotification(reminderLogSheet, teacherName, targetDateStr, notifType, sequence) {
  if (reminderLogSheet) {
    reminderLogSheet.appendRow([teacherName, targetDateStr, notifType, new Date(), sequence || 0]);
  }
}
 ...
 * @param {string} teacherName Nama guru.
 * @param {string} targetDateStr Tanggal target absensi (dd/MM/yyyy).
 * @param {string} notifType Tipe notifikasi.
 * @return {boolean} True jika sudah dikirim hari ini.
 */
function helper_hasSentNotificationToday(reminderLogSheet, teacherName, targetDateStr, notifType) {
  ...
  for (let i = logData.length - 1; i >= 1; i--) { // Cek dari data terbaru
    const row = logData[i];
    // Kolom: Nama_Guru (0), Target_Tanggal_Absen (1), Tipe_Notifikasi (2), Tanggal_Notifikasi_Terkirim (3)
    const logTeacher = String(row[0]).trim();
    const logTargetDate = String(row[1]).trim();
    const logNotifType = String(row[2]).trim();
    const logSentTimestamp = row[3]; // Bisa jadi string atau Date object
    ...
    if (logSentTimestamp instanceof Date) {
        ...
        try { logSentDate = new Date(logSentTimestamp); } catch(e) { /* abaikan jika parse gagal */ }
    } else if (typeof logSentTimestamp === 'number') { // Jika timestamp epoch
        logSentDate = new Date(logSentTimestamp);
    }
    ...
    if (logTeacher === teacherName && 
        logTargetDate === targetDateStr && 
        logNotifType === notifType && 
        logSentDate && 
        logSentDate.getTime() === todayLog.getTime()) {
      return true; 
    }
  }
  return false;
...
// ============== AKHIR FUNGSI BARU UNTUK NOTIFIKASI PENGINGAT ABSENSI ==============


// Pastikan fungsi `ensureSheet` Anda sudah ada dan berfungsi dengan baik
// Jika belum, ini contoh sederhananya:
/*
function ensureSheet(sheetName, headers = []) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(sheetName);
  if (!sheet) {
    ...
    if (headers.length > 0) {
      sheet.appendRow(headers);
      sheet.getRange(1, 1, 1, headers.length).setFontWeight("bold").setHorizontalAlignment("center");
      if (sheetName === REMINDER_LOG_SHEET_NAME) { // Auto-adjust column widths for log sheet
        sheet.autoResizeColumn(1); sheet.autoResizeColumn(2); sheet.autoResizeColumn(3);
        sheet.setColumnWidth(4, 180); // Tanggal Notifikasi
      ...
      sheet.appendRow(headers);
      sheet.getRange(1, 1, 1, headers.length).setFontWeight("bold").setHorizontalAlignment("center");
       if (sheetName === REMINDER_LOG_SHEET_NAME) {
        sheet.autoResizeColumn(1); sheet.autoResizeColumn(2); sheet.autoResizeColumn(3);
...
*/
// Pastikan Anda sudah memiliki fungsi `ensureSheet` yang serupa.
// Jika Anda menggunakan versi `ensureSheet` dari kode sebelumnya, itu sudah cukup.

// Anda perlu menjalankan fungsi `createOrEnsureDailyReminderTrigger()` sekali secara manual
// dari editor Apps Script untuk mengaktifkan pemicu otomatisnya.
// Contoh:
// function setupMyTriggers() {
//   createOrEnsureDailyReminderTrigger();
//   createOrEnsureCleanupTrigger(); // Jika Anda punya fungsi cleanup lain
//   createTriggerForCleanupIfNotExists(); // Jika Anda punya fungsi cleanup lain
// }

// Di dalam file code.gs.html

// Variabel global untuk nama sheet (pastikan sudah ada atau tambahkan jika belum)
// const CEKLOK_MANUAL_SHEET_NAME = "Ceklok_Manual";
// const KALENDER_SHEET_NAME = "Kalender_Pendidikan";
// const OPSIKETLIBUR_SHEET_NAME = "Opsi_Keterangan_Libur";
...
function getTeacherMonthlySummary(teacherName, monthYear) {
  try {
    ...
    let summary = {
      ...
      completedChecklok: 0, // Akan dihitung nanti dari penjumlahan
      present: 0,
      schoolHoliday: 0,
      otherValidReason: 0,
      absent: 0,
    ...
    for (let day = 1; day <= lastDayOfMonth; day++) {
      ...
      if (dayOfWeek === 0) { // Minggu
        isWorkingDay = false;
      } else if (liburMap[dateStr]) { // Libur Nasional/Sekolah dari Kalender_Pendidikan
        isWorkingDay = false;
      }

      let entryProcessedForDay = false; 
      for (let k = 1; k < allCeklokData.length; k++) {
          ...
          if (String(row[idxNama]).trim() === teacherName &&
              ...
              if (jamMasuk && jamPulang) {
                  if(isWorkingDay) summary.present++; 
              } else if (ketManual) {
                  ...
                  if(isWorkingDay) summary.incomplete++; 
              } else { 
                  if (isWorkingDay && currentDate < today) {
                      summary.absent++;
                  }
              }
              entryProcessedForDay = true;
              break;
          }
      }
      ...
      if (isWorkingDay) {
        ...
        let dayHasPassedOrIsToday = currentDate <= today;
        if (dayHasPassedOrIsToday) {
            summary.effectiveWorkingDaysPassed++;
            if (!entryProcessedForDay && currentDate < today) {
                summary.absent++;
            }
        }
      }
    ...
    // --- HITUNG completedChecklok DARI STATISTIK YANG SUDAH ADA ---
    summary.completedChecklok = summary.present + summary.schoolHoliday + summary.otherValidReason; //
    // --- AKHIR PERHITUNGAN completedChecklok ---

    // Kalkulasi Persentase Pengisian Ceklok BARU
    if (summary.totalWorkingDaysInMonth > 0) {
      ...
      summary.attendancePercentage = 0; // Jika tidak ada hari kerja dalam sebulan, persentase 0
    }
 ...
 */
function lockMonth(monthYearToLock) {
  try {
